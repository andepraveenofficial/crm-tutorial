...
class GetRecords {
    ...
    static async getRecords(moduleAPIName, fieldNames=null, options={pageNo:1, recordsPerPage:100}) {

      // custom data
      const {pageNo, recordsPerPage} = options
      //let moduleAPIName = "module_api_name";
        ...
        */
        ...
        // await paramInstance.add(ZOHOCRMSDK.Record.GetRecordsParam.SORT_BY, "Email"); // If you want uncomment
        await paramInstance.add(ZOHOCRMSDK.Record.GetRecordsParam.SORT_ORDER, "desc");
        await paramInstance.add(ZOHOCRMSDK.Record.GetRecordsParam.PAGE, pageNo);
        await paramInstance.add(ZOHOCRMSDK.Record.GetRecordsParam.PER_PAGE, recordsPerPage);
        /*
        ...
        */
        // await paramInstance.add(ZOHOCRMSDK.Record.GetRecordsParam.TERRITORY_ID, "34096430505351");  // Need to Fix Error
        ...
        // await headerInstance.add(ZOHOCRMSDK.Record.GetRecordsHeader.IF_MODIFIED_SINCE, new Date("2020-01-01T00:00:00+05:30"));  // If uncomment the modules comes after given date
                ...
                if (responseObject instanceof ZOHOCRMSDK.Record.ResponseWrapper) {
                    ...
                    records.forEach(record => {
                      ...
                      let recordData = {
                        ...
                        if (modifiedBy != null) {
                          recordData.modifiedBy = {
                            id: modifiedBy.getId()?.toString(),
                        ...
                        if (fieldNames) {
                          fieldNames.forEach((field) => {
                            recordData.fields[field.trim()] = record.getKeyValue(field.trim());
                          });
                        }
                        ...
                        // Add to data in each loop
                        result.data.push(recordData)
                    });
                    let info = responseObject.getInfo();
                    if (info) {
                      result.info = {
                          perPage: info.getPerPage(),
                          count: info.getCount(),
                          page: info.getPage(),
