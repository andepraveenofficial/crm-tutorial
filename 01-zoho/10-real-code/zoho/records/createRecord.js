...
class CreateRecord {
    ...
    static async createRecord(moduleAPIName, data) {
        ...
        for (let [key, value] of Object.entries(data)) {
            try {
              // Get the field constant from ZOHOCRMSDK
              const fieldConstant = ZOHOCRMSDK.Record.Field[moduleAPIName]?.[key.toUpperCase()];
              
              if (fieldConstant) {
                record1.addFieldValue(fieldConstant, value);
              } else {
        ...
        // record1.addFieldValue(ZOHOCRMSDK.Record.Field.Leads.LAST_NAME, data.LAST_NAME);// checking
        ...
        */
        ...
        await records.push(record1);
        await request.setData(records);
        /*
